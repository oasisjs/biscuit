...
import type { GatewayBot } from '@biscuit/api-types';
import type { GatewayManager } from './gatewayManager';

import { createGatewayManager } from './gatewayManager';
...
export function activate(resharder: Resharder): void {
	...
	resharder.intervalId = setInterval(async () => {
		...
		const percentage =
			((result.shards - resharder.gateway.manager.totalShards) /
				resharder.gateway.manager.totalShards) *
			100;
		// Less than necessary% being used so do nothing
		if (percentage < resharder.percentage) {
			return;
		}

		// Don't have enough identify rate limits to reshard
		if (result.sessionStartLimit.remaining < result.shards) {
			return;
		}
...
export async function reshard(resharder: Resharder, gatewayBot: GatewayBot) {
	...
	if (resharder.useOptimalLargeBotSharding) {
		...
		tmpManager.manager.totalShards =
...
export async function resharderIsPending(gateway: GatewayManager) {
