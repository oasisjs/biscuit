	...
	iconURL(
		...
		if (this.iconHash) {
			return Util.formatImageURL(
				Routes.GUILD_ICON(
					this.id,
					Util.iconBigintToHash(this.iconHash)
				),
				options.size,
				options.format
			);
...
export class AnonymousGuild extends BaseGuild implements Model {
	...
	constructor(session: Session, data: DiscordGuild) {
		...
		this.bannerHash = data.banner
			? Util.iconHashToBigInt(data.banner)
			: undefined;
	...
	splashURL(
		...
		if (this.splashHash) {
			return Util.formatImageURL(
				Routes.GUILD_SPLASH(
					this.id,
					Util.iconBigintToHash(this.splashHash)
				),
				options.size,
				options.format
			);
	...
	bannerURL(
		...
		if (this.bannerHash) {
			return Util.formatImageURL(
				Routes.GUILD_BANNER(
					this.id,
					Util.iconBigintToHash(this.bannerHash)
				),
				options.size,
				options.format
			);
...
export class Guild extends BaseGuild implements Model {
	...
	async editBotNickname(options: {
		...
		const result = await this.session.rest.runMethod<
			{ nick?: string } | undefined
		>(this.session.rest, 'PATCH', Routes.USER_NICK(this.id), options);
	...
	async fetchInvite(
		inviteCode: string,
		options: Routes.GetInvite
	): Promise<Invite> {
		const inviteMetadata =
			await this.session.rest.runMethod<DiscordInviteMetadata>(
				this.session.rest,
				'GET',
	...
	async getActiveThreads(): Promise<Omit<ReturnThreadsArchive, 'hasMore'>> {
		const { threads, members } =
			await this.session.rest.runMethod<DiscordListActiveThreads>(
				this.session.rest,
				'GET',
	...
	static async create(
		...
		const guild = await session.rest.runMethod<DiscordGuild>(
			session.rest,
			'POST',
			Routes.GUILDS(),
			{
	...
	async fetchWidgetSettings(): Promise<GuildWidgetSettings> {
		const widget =
			await this.session.rest.runMethod<DiscordGuildWidgetSettings>(
				this.session.rest,
				'GET',
