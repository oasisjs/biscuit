...
async function resume(shard) {
    ...
    if (shard.isOpen()) {
        shard.close(types_1.ShardSocketCloseCodes.ResumeClosingOldConnection, 'Reconnecting the shard, closing old connection.');
    }
    ...
    await shard.connect();
    shard.send({
        op: api_types_1.GatewayOpcodes.Resume,
        d: {
