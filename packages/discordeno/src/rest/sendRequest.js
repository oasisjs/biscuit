...
async function sendRequest(rest, options) {
    try {
        ...
        const newURL = options.url.startsWith(constants_1.BASE_URL)
            ? options.url
            : `${constants_1.BASE_URL}/v${rest.version}/${options.url}`;
        ...
        if (response.status < 200 || response.status >= 400) {
            ...
            switch (response.status) {
                case api_types_1.HTTPResponseCodes.BadRequest:
                    error =
                        "The options was improperly formatted, or the server couldn't understand it.";
                    break;
                case api_types_1.HTTPResponseCodes.Unauthorized:
                    error = 'The Authorization header was missing or invalid.';
                    break;
                case api_types_1.HTTPResponseCodes.Forbidden:
                    error =
                        'The Authorization token you passed did not have permission to the resource.';
                    break;
                case api_types_1.HTTPResponseCodes.NotFound:
                    error =
                        "The resource at the location specified doesn't exist.";
                    break;
                case api_types_1.HTTPResponseCodes.MethodNotAllowed:
                    error =
                        'The HTTP method used is not valid for the location specified.';
                    break;
                case api_types_1.HTTPResponseCodes.GatewayUnavailable:
                    error =
