...
import type { DiscordMemberWithUser } from '@biscuit/api-types';
...
export class Member implements Model {
	constructor(
		...
		this.avatarHash = data.avatar
			? Util.iconHashToBigInt(data.avatar)
			: undefined;
	...
	avatarURL(
		options: { format?: ImageFormat; size?: ImageSize } = { size: 128 }
	): string {
		let url: string;
		...
		if (!this.avatarHash) {
			url = Routes.USER_DEFAULT_AVATAR(
				Number(this.user.discriminator) % 5
			);
		} else {
			url = Routes.USER_AVATAR(
				this.user.id,
				Util.iconBigintToHash(this.avatarHash)
			);
		}
