...
import { HTTPResponseCodes } from '@biscuit/api-types';
...
export async function sendRequest<T>(
	...
	try {
		...
		rest.debug(
			`[REST - fetching] URL: ${options.url} | ${JSON.stringify(options)}`
		);

		const newURL = options.url.startsWith(BASE_URL)
			? options.url
		...
		rest.debug(
			`[REST - fetched] URL: ${options.url} | ${JSON.stringify(options)}`
		);
		...
		if (response.status < 200 || response.status >= 400) {
			...
			if (response.status !== 429) {
				...
				if (
					...
					rest.debug(
						`[REST - RetriesMaxed] ${JSON.stringify(options)}`
					);
		...
		if (response.status === 204) {
			rest.debug(
				`[REST - FetchSuccess] URL: ${options.url} | ${JSON.stringify(
					options
				)}`
			);
			options.respond?.({
