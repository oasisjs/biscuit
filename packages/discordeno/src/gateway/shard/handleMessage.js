...
async function handleMessage(shard, message_) {
    ...
    if (shard.gatewayConfig.compress && message instanceof Blob) {
        ...
        message = decoder.decode((0, node_zlib_1.inflateSync)(buff));
    }
    ...
    switch (messageData.op) {
        case api_types_1.GatewayOpcodes.Heartbeat: {
        ...
        case api_types_1.GatewayOpcodes.Hello: {
            const interval = messageData.d.heartbeat_interval;
            shard.startHeartbeating(interval);
            if (shard.state !== types_1.ShardState.Resuming) {
                ...
                shard.bucket = (0, bucket_1.createLeakyBucket)({
        ...
        case api_types_1.GatewayOpcodes.HeartbeatACK: {
        ...
        case api_types_1.GatewayOpcodes.Reconnect: {
            //   gateway.debug("GW RECONNECT", { shardId });
            shard.events.requestedReconnect?.(shard);
            await shard.resume();
            break;
        }
        case api_types_1.GatewayOpcodes.InvalidSession: {
            ...
            await (0, delay_1.delay)(Math.floor((Math.random() * 4 + 1) * 1000));
    ...
    if (messageData.t === 'RESUMED') {
        ...
        shard.state = types_1.ShardState.Connected;
