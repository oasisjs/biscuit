...
export interface CreateContextApplicationCommand
	extends Omit<CreateApplicationCommand, 'options'> {
...
export class Session extends EventEmitter {
	...
	async editProfile(nick?: string, avatarURL?: string | null): Promise<User> {
		...
		const user = await this.rest.runMethod<DiscordUser>(
			this.rest,
			'PATCH',
			Routes.USER(),
			{
	...
	async fetchUser(id: Snowflake): Promise<User | undefined> {
		const user: DiscordUser = await this.rest.runMethod<DiscordUser>(
			this.rest,
			'GET',
			Routes.USER(id)
		);
	...
	deleteApplicationCommand(
		...
		return this.rest.runMethod<undefined>(
			...
			'DELETE',
			guildId
				? Routes.GUILD_APPLICATION_COMMANDS(
						this.applicationId,
						guildId,
						id
				  )
	...
	updateApplicationCommandPermissions(
		...
		return this.rest.runMethod<DiscordGuildApplicationCommandPermissions>(
			this.rest,
			'PUT',
			Routes.GUILD_APPLICATION_COMMANDS_PERMISSIONS(
				this.applicationId,
				guildId,
				id
			),
	...
	fetchApplicationCommandPermissions(
		guildId: Snowflake
	): Promise<DiscordGuildApplicationCommandPermissions[]> {
		return this.rest.runMethod<DiscordGuildApplicationCommandPermissions[]>(
			this.rest,
	...
	isContextApplicationCommand(
		...
		return (
			cmd.type === ApplicationCommandTypes.Message ||
			cmd.type === ApplicationCommandTypes.User
		);
	}

	async start(): Promise<void> {
		const getGatewayBot = () =>
			this.rest.runMethod<DiscordGetGatewayBot>(
				this.rest,
				'GET',
				Routes.GATEWAY_BOT()
			);
		...
		if (!Object.keys(this.options.gateway?.data ?? {}).length) {
			...
			this.gateway.gatewayBot = {
				...
				sessionStartLimit: {
					...
					maxConcurrency:
						nonParsed.session_start_limit.max_concurrency,
				},
			};
