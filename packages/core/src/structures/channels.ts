...
export abstract class BaseChannel implements Model {
	...
	isThread(): this is ThreadChannel {
		return (
			this.type === ChannelTypes.GuildPublicThread ||
			this.type === ChannelTypes.GuildPrivateThread
		);
	}
...
export class TextChannel {
	...
	async fetchPins(): Promise<Message[] | []> {
		...
		return messages[0]
			? messages.map((x: DiscordMessage) => new Message(this.session, x))
			: [];
	}
	...
	sendMessage(options: CreateMessage): Promise<Message> {
		return Message.prototype.reply.call(
			{ channelId: this.id, session: this.session },
			options
		);
	}
...
export class GuildChannel extends BaseChannel implements Model {
	...
	async getArchivedThreads(
		options: Routes.ListArchivedThreads & {
...
export interface DMChannel
	extends Omit<TextChannel, 'type'>,
		Omit<BaseChannel, 'type'> {}
...
export class ThreadChannel extends GuildChannel implements Model {
	...
	removeMember(memberId: Snowflake = this.session.botId): Promise<void> {
		return ThreadMember.prototype.quitThread.call(
			{ id: this.id, session: this.session },
			memberId
		);
	}

	fetchMember(
		memberId: Snowflake = this.session.botId
	...
	async fetchMembers(): Promise<ThreadMember[]> {
		const members = await this.session.rest.runMethod<
			DiscordThreadMember[]
...
export interface ThreadChannel
	extends Omit<GuildChannel, 'type'>,
		Omit<TextChannel, 'type'> {}
