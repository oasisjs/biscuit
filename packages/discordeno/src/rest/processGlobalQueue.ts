...
export async function processGlobalQueue(rest: RestManager) {
	...
	while (rest.globalQueue.length) {
		...
		if (rest.globallyRateLimited) {
			setTimeout(() => {
				rest.debug(
					`[REST - processGlobalQueue] Globally rate limited, running setTimeout.`
				);
		...
		if (
			...
			rest.maxInvalidRequests - rest.invalidRequestsSafetyAmount
		) {
			setTimeout(() => {
				const time =
					rest.invalidRequestsInterval -
					(Date.now() - rest.invalidRequestFrozenAt);
				rest.debug(
		...
		if (urlResetIn || bucketResetIn) {
			...
			setTimeout(() => {
				rest.debug(
					`[REST - processGlobalQueue] rate limited, running setTimeout.`
				);
